...
Updates an existing UI plugin instance in the current tenant from the workspace manifest at `./sp-ui-plugin.json`.
...
```bash
# Update from ./sp-ui-plugin.json
SAIL_EXPERIMENTAL_UI_PLUGINS=1 sail ui-plugins update

# Preview the payload (and check the instance exists) without updating
SAIL_EXPERIMENTAL_UI_PLUGINS=1 sail ui-plugins update --dry-run

# Restrict the plugin to yourself on every slot
SAIL_EXPERIMENTAL_UI_PLUGINS=1 sail ui-plugins update --private

# Restrict the plugin to specific users on every slot
SAIL_EXPERIMENTAL_UI_PLUGINS=1 sail ui-plugins update --restrict-to-users 2c9180...a1,2c9180...b2

# Print the raw UMS response on success
SAIL_EXPERIMENTAL_UI_PLUGINS=1 sail ui-plugins update --json
```
