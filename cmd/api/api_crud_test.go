...
func TestNewCRUDCmd(t *testing.T) {
	...
	// Get the transform ID from the response using JSONPath
	transformID, err := jsonpath.EvaluateJSONPathToString(createBuffer.Bytes(), "$.id")
	if err != nil {
		t.Fatalf("Failed to get transform ID from response: %v", err)
	}
	...
	// Verify the retrieved transform matches what we created using JSONPath
	retrievedName, err := jsonpath.EvaluateJSONPathToString(getBuffer.Bytes(), "$.name")
	if err != nil {
		t.Fatalf("Failed to get name from response: %v", err)
	}
	...
	// Verify the sourceName was updated in the PUT response using JSONPath
	putSourceName, err := jsonpath.EvaluateJSONPathToString(updateBuffer.Bytes(), "$.attributes.sourceName")
	if err != nil {
		t.Fatalf("Failed to get sourceName from response: %v", err)
	}
	if putSourceName != "Updated Workday" {
	...
	// Verify the retrieved transform matches our updates using JSONPath
	retrievedName, err = jsonpath.EvaluateJSONPathToString(getBuffer.Bytes(), "$.name")
	if err != nil {
		t.Fatalf("Failed to get name from response: %v", err)
	}
